...
**Version**: 1.12 | **Author**: Rebecca | **Date**: 2026-06-04 | **Status**: DRAFT
...
| **Fiat Account Mapping Reference Table** | Unified table mapping payment channel account numbers to client accounts (GLDB, SGB, TransferMate, Tazapay, etc.). Stores client identity (name, participant code, member ID) with account status checks. Bulk upload for GLDB only; individual CRUD via new CAMP Admin page. |
...
| **Pending Deposit Queue** | Replaced by CAMP Task Center integration (§7.7.3). Deposit tasks (matching, status exception, missing fields, classification) managed as Task Center tasks. |
...
4. GLDB has no committed timeline for fixing webhook to return real MCA account numbers. This does not affect the deposit STP architecture: the Fiat Account Mapping Reference Table (§7.4) always maps the channel account number (from the GLDB webhook/API response) to the real MCA account number, regardless of whether GLDB returns internal account numbers or real MCA numbers. Whether GLDB eventually fixes the field is purely a question of which field path the backbone team reads as the account identifier — the mapping table configuration selects the source field per channel, so no pipeline logic changes are required either way. The STP pipeline is fully reliant on this mapping table for GLDB account resolution.
...
4. **Webhook Processing** (Payment Channel → Backbone → Payment Backend): When funds land, the payment channel sends a webhook notification. Backbone validates and forwards it to Payment Backend. The backend then queries the backbone for full transaction details, which are retrieved from the payment channel and returned through the chain.

5. **Deposit Order & Status Reconciliation** (Payment Backend): A deposit order is created and the system checks the final status. A scheduled job periodically checks for any missing transactions (in case a webhook was missed) and re-checks the status of pending transactions until a terminal status is reached. On success, the system identifies the recipient member from transaction details.
...
| valueDate > today | Schedule at 00:00 on value date |
...
> **Note**: Reconciliation details will be covered in a separate PRD. This PRD targets fiat deposit automation, and reconciliation processes should already be in place. So far no change to reconciliation has been identified.

- EOD reconciliation includes: bank statement balance, CAMP ledger balance (incl. Unclaimed Funds), and in-flight pending deposits
...
> **GLDB Omnibus Account Mapping (Illustrative Example)**: Channel-specific account number resolution is achieved through the **Fiat Account Mapping Reference Table** (§7.4), not hardcoded per-channel logic. The following illustrates how this applies to GLDB: For GLDB omnibus accounts, the account number received in the webhook (GLDB internal account number) typically differs from the real MCA (MetaComp Client Account) number. The mapping table entry links the GLDB internal account number to the correct MCA account number and client record. The same mechanism applies to any channel — the mapping table provides the resolution layer, and Step 2 (Account Match) queries the table before Step 3 (Reference Code Match).
...
- [ ] Account Status Check (Step 4) blocks auto-credit for non-Active statuses (Initial / Suspended / Closed) and routes deposit to Task Center as a Status Exception with the blocking status clearly displayed
...
- For multiple-parent VA scenarios:
  ...
  - If the VA has no reference code assigned, behavior depends on the VA structure:
    - **Single-parent VA**: the VA itself is the unique identifier. Proceed to Step 4 (Account Status Check). No disambiguation needed.
...
- **Fallback Strategy**: Task Center task with Ops manual review for any rule that cannot resolve.
...
> **Phase 1 Note**: This strategy depends on the `NameMatchService` module (§7.2), which is included in Phase 1. See §7.2 for the NameMatchService scope and PSN04 reporting rationale.
...
- Saved sender details can be entered through: (1) **Partner API**: API-integrated partners can pre-save known sender details via the existing Pre-save 3rd Party Fiat Account API endpoint (§7.12). (2) **Client self-service**: the client can pre-save known sender names via the AgentX Portal page.
...
| High | Single-strategy match with score ≥ 0.80 |
| Medium | Single-strategy match with score between 0.50 and 0.80 (exclusive upper bound) |
...
- Ops selects the correct recipient from the list in the Task Center, which triggers the deposit pipeline to re-enter at Step 4 (Account Status Check) → Step 6 (Classification) → Step 7 (Mandatory Fields Validation) → Step 8 (Screening) → Step 9 (Credit).
...
**Performance Consideration**: Strategies 3 (Sender Name Fuzzy Match, Phase 2) and 4 (Saved Sender Details Match) execute against all CAMP client records. To ensure acceptable response times:
...
1. Resolve channel-specific account numbers to CAMP client accounts (MCA numbers)
2. Display client identity details (name, participant code, member ID) for Ops verification
...
| Channel Account Number | Text | Account number as sent in the payment channel's webhook. Primary lookup key for deposit matching. |
| MCA Account Number | Text | Real MetaComp Client Account number that the channel account maps to. |
...
| Client Name | Lookup | Client name auto-populated via CAMP lookup against MCA number. Read-only. |
| Participant Code | Lookup | Client participant code auto-populated from CAMP. Read-only. |
| Member ID | Lookup | Client member ID auto-populated from CAMP. Read-only. |
...
| Condition | Result |
|-----------|--------|
| Both Participant Status and Member Status are **Active** (or equivalent) | ✅ Proceed (rule engine Step 4 — Account Status Check — passes, continuing to 1st/3rd Party Classification). |
...
| Feature | Detail |
|---------|--------|
| Scope | GLDB only — bulk upload is for GLDB channel account-to-MCA mappings. |
| Upload Mode | Append/update — does NOT replace the entire table. Existing non-GLDB entries are unaffected. Supports adding new rows and updating existing GLDB rows. **Update rule**: if (Payment Channel + Channel Account Number) matches an existing row, the row is **updated** with new values for the remaining columns (MCA Account Number, Currency, Account Type, Participant Code/Member ID). If no match, a **new row** is inserted. Uniqueness is enforced at insert time — if a duplicate key combination would be created by an update, the update is rejected with an error. **Duplicate handling**: Before upload, Ops selects a duplicate handling mode — **Ignore** (skip duplicate rows, keep existing data unchanged) or **Overwrite** (update existing rows with new values). |
| Template | Excel template with columns: Payment Channel, Channel Account Number, MCA Account Number, Currency, Account Type (Fiat / Investment Fiat), Participant Code (or Member ID). Reference Code is **not** included in the bulk upload — it is a system-generated field. At least one of Participant Code or Member ID must be provided for client identification. If both are provided but do not reference the same client, the row is rejected. Client identity fields (Client Name, Statuses) are auto-populated on upload via CAMP lookup. |
| Validation | All rows validated before commit. Participant Code and Member ID consistency enforced: if both are provided, they must reference the same client or the row is rejected. Invalid or unresolvable rows rejected with error messages. Valid rows committed together. No partial commits. |
...
| Channel Account Number | Text input | Partial match |
| Reference Code | Text input | Partial match |
| MCA Account Number | Text input | Partial match |
| Client Name | Text input | Partial match |
| Participant Code | Text input | Exact or partial match |
| Member ID | Text input | Exact or partial match |
...
| Channel Account Number | Text, truncated with full-value tooltip on hover |
| Reference Code | Text |
| MCA Account Number | Text |
...
| Action | Behaviour |
|--------|-----------|
| **Add New Entry** | Form with all schema fields including Account Type (Fiat / Investment Fiat) except Reference Code (system-generated, populated automatically). Client identity fields auto-populate on MCA Account Number entry via CAMP lookup (debounced). |
| **Edit Entry** | For GLDB entries: Channel Account Number and MCA Account Number are editable to allow account info corrections (GLDB webhooks do not return real MCA numbers — see §5.3 Assumption 4 — so Ops may need to correct mappings after initial upload). For non-GLDB entries: channel account numbers are read-only. Mapping Status (Active/Inactive toggle) is always editable. Client identity fields (Client Name, Participant Code, Member ID, Statuses) are always read-only after creation. |
...
**Functional Rules**:
- Duplicate (Payment Channel + Channel Account Number + Account Type) combinations are rejected on add or upload. Since reference codes are system-generated and not user-editable, each channel account number maps to at most one active reference code per Account Type for the same payment channel. For Named VAs (where Reference Code is null), uniqueness is still enforced on Payment Channel + Channel Account Number + Account Type. The same user may have separate entries for Fiat vs Investment Fiat accounts with different reference codes.
- Client identity fields are auto-populated from CAMP (via MCA number lookup) and are read-only.
...
**Acceptance Criteria**:
- [ ] Webhook channel account number → table lookup → MCA number → client identification (including participant code & member ID)
...
- [ ] Deferred credit scheduled and executed at 00:00 on value date
- [ ] Missing value date handled by crediting immediately
- [ ] Lark notification sent when deferred credit record created (§7.8 — Value Date Deferred template)
...
- **Fallback Strategy**: Missing value date → credit immediately. Default deferred credit time 00:00 on value date.
...
| Internal GLDB Account | Real MCA Account | Notes |
|----------------------|------------------|-------|
...
| GLDB omnibus mapping fallthrough | Fiat Account Mapping Reference Table has no entry for the GLDB account, and reference code is absent |
...
|-----------|---------|-------------|--------------------------------------|
| `DEPOSIT_RECIPIENT_MATCHING` | Rule engine Step 5 fallthrough (unidentified deposit) | Ops reviews Potential Match Discovery candidate list (§7.1.4) or uses manual CAMP client search to select the correct recipient | `PROCESS_DEPOSIT` (Ops Analyst) |
| `DEPOSIT_STATUS_EXCEPTION` | Rule engine Step 4 (Account Status Check) failure | Account status is Initial/Suspended/Closed. Ops must resolve underlying status or escalate. "Retry" action re-enters pipeline at Step 6. | `PROCESS_DEPOSIT` (Ops Analyst) / `MANAGE_ACCOUNT_STATUS` (Ops Admin) |
...
|-------------------|------------------------|
| **Ops selects recipient** from candidate list or manual search | Deposit module validates account status (Active?), then advances from Step 4 (Account Status Check) → Step 6 (1st/3rd Party Classification) → Step 7 (Mandatory Fields Validation) → Step 8 (Screening) → Step 9 (Credit) |
...
| **Ops classifies 1st/3rd party** | Deposit module updates classification, proceeds to Step 8 |
...
4. **Action Buttons**: "Confirm Recipient & Proceed" (enabled only after recipient selected), "Reject/Refund", "Add Note", **"View in Order Center"** (link to the deposit record in CAMP Admin Full Transaction Query page — always available regardless of task state). Confirmation dialog on proceed: *"Are you sure this is the correct recipient? This action will proceed to compliance screening and crediting."*
...
6. **Unclaimed Ageing Progress Bar** (for `DEPOSIT_RECIPIENT_MATCHING` tasks on unclaimed deposits): Visual timeline showing current age in days. The threshold for the ageing Lark notification is configurable (pending Ops confirmation). No fixed milestone markers — the bar reflects actual elapsed time continuously.

7. **Notes Section**: Ops can add free-text notes. Persisted in audit trail.
...
|---|---|---|
| `processing.auto` / `processing.deferred` | No | System handles automatically |
...
| **CAMP Admin Transaction Query Page (§7.11)** | The Full Transaction Query tab (§7.11.1) remains as the historical data query surface for all deposits. The Pending Transaction Query tab (§7.11.2) provides a read-only monitoring and ageing surface — all operational actions are handled through the CAMP Task Center (§7.7.3). |
...
1. Ops locates the unclaimed deposit in the Pending Transaction Query tab (§7.11.2).
...
|-------|---------|----------|
| **Informational Group** | Ops awareness only — no action required | Missing Mandatory Fields, Recipient Confirmed Awaiting Value Date, Funds Credited |
...
| **Tech Alert Group** | Engineering team notification — system-level issues requiring investigation | VisionX API timeout / retry exhaustion, Rule Engine system errors |
...
Action Required: No action required — recipient identified and all checks passed. Funds will auto-credit at 00:00 on the value date.
...
Cronjob Retry Attempts: 3
Action Required: Manual investigation — webhook received but transaction details could not be retrieved after 3 cronjob retry attempts (30 minutes). Check the payment channel portal for the transaction and verify its status.
...
### 7.10 Pending Deposit Queue (Deprecated — See §7.7.3)

**Description**: This section is **deprecated**. The Pending Deposit Queue concept has been replaced by the CAMP Task Center integration described in §7.7.3.

All deposit records requiring Ops action are now managed through the CAMP Task Center. The Task Center provides a unified, function-access-filtered inbox with:

- **Deposit-specific task types** — `DEPOSIT_RECIPIENT_MATCHING`, `DEPOSIT_STATUS_EXCEPTION`, `DEPOSIT_3RD_PARTY_CLASSIFICATION` (see §7.7.3 for complete specifications)
- **Pre-populated fields** — all webhook fields carried as task metadata; Ops only fills genuinely missing fields
- **Potential Match Discovery candidates** and manual search fallback
- **Action lifecycle** — confirm recipient, retry status exception, classify 1st/3rd party
- **Full audit trail** — all Ops actions logged with identity and timestamp

**Cross-Reference**: See §7.7.3 Task Center Integration for deposit task type definitions, task creation API, action lifecycle, task status mapping, and Task Center detail view specifications. SLA tracking, priority calculation, and reminders are handled by the CAMP Task Center's standard mechanism — no additional configuration required in this module.

---

### 7.11 CAMP Admin New Transaction Query Page

**Description**: Add a new transaction query page in CAMP Admin with two tabs — **Pending Transaction Query** and **Full Transaction Query**. Existing CAMP Admin pages remain unchanged.

#### 7.11.1 Full Transaction Query Tab
...
|-----------|------|-------------|
| Transaction ID | Text | Exact match or partial match |
| Client Name / Participant Code | Text | Search by client name or participant code |
| Payment Channel | Dropdown | GLDB, SGB, TransferMate, Tazapay, etc. |
| Amount Range | Range | Min / Max amount |
| Currency | Dropdown | SGD, USD, etc. |
| Date Range | Date picker | Webhook receipt date or value date range |
| Status | Dropdown | Auto-credited, Pending Ops, Pending Compliance, Refunded, RFI, Unclaimed, etc. |
| Sender Name | Text | Sender name from transaction details |
| Reference Code | Text | Reference code carried in the transfer |

**Display Columns** (all transaction fields must be available for display and export):

| Column | Source |
|--------|--------|
| Transaction ID | Webhook / system generated |
| Payment Channel | Auto-tagged from webhook source |
| Amount | From transaction details |
| Currency | From transaction details |
| Value Date | From transaction details |
| Credit Date | System timestamp of credit execution |
| Sender Name | From transaction details |
| Sender Account No | From transaction details |
| Sender Bank SWIFT BIC | From transaction details |
| Sender Bank Name | From transaction details |
| Beneficiary Name | From recipient identification |
...
- Ops can customise which columns to show/hide
...
#### 7.11.2 Pending Transaction Query Tab

**Purpose**: Query and operational surface for deposits requiring Ops action. Task-management actions (recipient selection, status resolution, 3rd party classification) are performed in the CAMP Task Center (§7.7.3) via the "View in Task Center" link. Direct actions available in this tab include: "Mark as Refunded" for prolonged unclaimed deposits (§7.7.8) and adding deposit-level notes. This tab provides deposit-centric visibility and ageing tracking alongside these targeted actions.

> **Design Decision**: Task-management actions (recipient matching, classification, status retry) are performed exclusively via the CAMP Task Center to avoid duplicate entry points. However, deposit-level actions that do not correspond to a Task Center task type — specifically "Mark as Refunded" for manual refund recording and adding deposit notes — are available directly in this tab. The tab's primary value is: (1) multi-dimensional query across all pending deposits, (2) ageing monitoring with alerts, (3) targeted direct actions for unclaimed deposit resolution, and (4) export for reconciliation/reporting.

**Query Conditions**: Same multi-dimensional filtering as Full Transaction Query tab (§7.11.1).
...
- **Ageing indicator**: each row includes a colour-coded ageing badge reflecting elapsed days (exact colour-code thresholds TBD by Ops)
- **Ageing progress bar** (for unclaimed deposits): visual timeline showing current age in days; no fixed milestone markers — reflects actual elapsed time continuously
...
- **Add Note**: available on all rows; note is attached to the deposit record (not the Task Center task)
...
- **Ageing Section**: ageing timeline with visual progress bar; current age, next notification threshold and date
...
- **Ageing alerts**: visual banner at top of page highlighting records approaching ageing thresholds (e.g. "3 unclaimed deposits over 90 days old")
...
- Ageing thresholds (flag/review/escalation days) are configurable without deployment
- Records with no Ops action for 90+ days display a prominent red ageing alert banner
...
| Transaction detail retrieval cronjob frequency | Every 10 minutes — scheduled job pulls transaction details from payment channel API for webhooks awaiting matched transactions |
...
- Duplicate webhook detection and idempotency: each webhook is deduplicated by its composite key **(payment_channel, transaction_id, event_type)**, since different payment channels may issue overlapping Transaction IDs and the same transaction may appear with different event types. If a duplicate webhook arrives (same composite key already processed or in-flight), the system ignores it and returns a success acknowledgement to the sender. Idempotency key is (payment_channel, transaction_id, event_type); the system maintains a configurable deduplication window (default 24 hours) to catch delayed duplicates. Concurrent duplicate arrivals are handled via database-level unique constraint on the composite key.
...
- Transaction detail retrieval cronjob runs every **10 minutes**. After receiving a webhook, the system attempts to back-query the payment channel API for full transaction details. If the transaction is not found, the system retries on each subsequent cronjob run. If the transaction is still not found after **3 consecutive cronjob runs** (i.e., 30 minutes), the system sends a Lark notification to the Ops Action Group for manual investigation. No deposit record or Task Center task is created for unmatched webhooks — the Lark notification is the only alert.
...
- **Shadow mode**: Phase 1 runs in production but does not auto-credit — logs decisions for validation. Duration: minimum 2 weeks or 200 production webhooks, whichever is later. *Gate: Shadow-mode decisions match manual Ops decisions on ≥95% of deposits. Zero false-negative 3rd party classifications.*
...
| GLDB webhook/API response field for account number may change format if GLDB eventually fixes it | Low | Low | Fiat Account Mapping Reference Table (§7.4) always resolves channel account numbers to MCA via the mapping table. GLDB's field format does not affect pipeline logic — the backbone team configures which field to read as the account identifier per channel in the mapping configuration. No pipeline changes needed regardless of GLDB's behavior. |
...
| # | Item | Owner | Priority |
|---|------|-------|----------|
| 1 | Provide complete ALF account exclusion list — full GLDB internal-to-MCA mapping. | Ops / Finance | P0 |
...
| 6 | Define v2 Partner API spec: push notification, pre-save fiat account, RFI response. | Product / Eng | P1 |
...
| 1.2 | 2026-05-26 | Added Step 4 — Account Status Check; non-Active statuses block auto-credit; Steps 4-8 renumbered to 5-9. Simplified §7.2 (classification restricted to sender name vs registered name only). Removed Action column from §7.2 (all deposits screened regardless of classification). |
...
### D. Deposit Record Schema

The deposit record uses the existing CAMP deposit table schema. Implementation teams should assess whether schema modifications are needed to accommodate the new fields referenced in this PRD (§7.1.1, §7.7.2, §7.11.1, §7.7.3) and make adjustments during development.
